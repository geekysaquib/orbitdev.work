...
export async function generateCommitMessage(diff: string, keys: ProviderKeys, preferred?: CloudProvider): Promise<{ ok: boolean; text?: string; error?: string }> {
...
export async function generatePrDescription(commitSubjects: string[], diff: string, keys: ProviderKeys, preferred?: CloudProvider): Promise<{ ok: boolean; text?: string; error?: string }> {
